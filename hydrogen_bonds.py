...
# A little adjustment
h_bonds_adv[a[0]] = np.concatenate( (h_bonds_adv[a[0]], np.NaN*np.ones( len(h_bonds_adv[a[1]]) - len(h_bonds_adv[a[0]]) ) ) )
print(len(h_bonds_adv[a[4]]))

"""
for a_val in a :
    if a_val==0.00 :
        plt.plot(time_adv, h_bonds_adv[a_val], label='a='+str(a_val), linewidth=3.00, c=cm.hot(a_val/(a_max+1)))
    else :
        plt.plot(time_adv, h_bonds_adv[a_val], label='a='+str(a_val), linewidth=1.50, c=cm.hot(a_val/(a_max+1)))
